...
class SqlStorage {
  ...
  async createTask(input) {
    ...
    await pool.request()
      ...
      .query(`
        INSERT INTO Tasks (id, title, description, status, priority, owner, dueDate, createdAt, updatedAt)
        VALUES (@id, @title, @description, @status, @priority, @owner, @dueDate, @createdAt, @updatedAt)
      `);
    return task;
  }
